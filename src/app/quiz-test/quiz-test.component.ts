...
//   ) {
//     this.timeInSecs = 55 * 60; // 1 minute in seconds
...
export class QuizTestComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    document.addEventListener('keydown', this.preventEscape);
    document.addEventListener('keydown', this.disableArrowNavigation); // Add this line
  ...
  ngOnDestroy(): void {
    ...
    document.removeEventListener('keydown', this.disableArrowNavigation); // Add this line
  ...
  disableArrowNavigation = (event: KeyboardEvent): void => {
    if (
      event.key === 'ArrowDown' ||
      event.key === 'ArrowUp' ||
      event.key === 'ArrowLeft' ||
      ...
      event.preventDefault(); // Prevent arrow keys from navigating the dropdown
    }
  };
  ...
  handleBeforeUnload = (event: BeforeUnloadEvent): void => {
    ...
    alert('Warning: Reloading or leaving the page will cause you to lose your progress!');
  };
